"""
QBM Training with Usul al-Fiqh Aligned Taxonomy

Uses filtered annotations with 33 TRUE behavior classes:
- Removes entity mentions (نبي, رسول, ملائكة)
- Removes state labels (مؤمن, كافر, منافق, etc.)
- Trains on actual behavioral patterns only

Layers:
2. Arabic Embeddings - Fine-tune on behavioral spans
3. Behavioral Classifier - 33-class classification (TRUE behaviors only)
4. Relation Extractor - Causal relations between behaviors
5. GNN Graph Reasoner - Behavioral co-occurrence graph
6. Domain Reranker - Cross-encoder for relevance
...
FILTERED_ANNOTATIONS = DATA_DIR / "annotations" / "tafsir_behavioral_annotations_filtered.jsonl"
...
def load_filtered_annotations() -> List[Dict]:
    """Load filtered TRUE behavior annotations."""
    annotations = []
    with open(FILTERED_ANNOTATIONS, 'r', encoding='utf-8') as f:
        for line in f:
            if line.strip():
                annotations.append(json.loads(line))
    logger.info(f"Loaded {len(annotations):,} TRUE behavior annotations")
    return annotations


# =============================================================================
# LAYER 2: ARABIC EMBEDDINGS (Usul-aligned)
# =============================================================================

def train_layer_2():
    """Train Arabic embeddings on TRUE behaviors only."""
    logger.info("=" * 60)
    logger.info("LAYER 2: Arabic Embeddings (Usul-aligned)")
    logger.info("=" * 60)
    ...
    annotations = load_filtered_annotations()
    ...
    # Based on Quranic categorization
    SALIH_BEHAVIORS = {beh_id for beh_id, concept in BEHAVIOR_TAXONOMY.items() 
                       if concept.is_true_behavior and concept.default_eval == ActionTextualEval.EVAL_SALIH}
    GHAYR_SALIH_BEHAVIORS = {beh_id for beh_id, concept in BEHAVIOR_TAXONOMY.items() 
                             if concept.is_true_behavior and concept.default_eval == ActionTextualEval.EVAL_GHAYR_SALIH}
    
    logger.info(f"Salih behaviors: {len(SALIH_BEHAVIORS)}")
    logger.info(f"Ghayr Salih behaviors: {len(GHAYR_SALIH_BEHAVIORS)}")
    ...
    for beh_id, texts in behavior_texts.items():
        ...
        # Determine if this is salih or ghayr_salih
        is_salih = beh_id in SALIH_BEHAVIORS
        
        # Hard negatives: same moral category but different behavior
        if is_salih:
            hard_neg_pool = [b for b in SALIH_BEHAVIORS if b != beh_id and b in behavior_texts]
        else:
            hard_neg_pool = [b for b in GHAYR_SALIH_BEHAVIORS if b != beh_id and b in behavior_texts]
        
        # Easy negatives: opposite moral category
        if is_salih:
            easy_neg_pool = [b for b in GHAYR_SALIH_BEHAVIORS if b in behavior_texts]
        else:
    ...
    # Evaluate with behavior pairs
    test_pairs = [
        # Same category (should be similar)
        ("BEH_HEART_ARROGANCE", "BEH_CHAR_OPPRESSION", True),  # Both ghayr_salih
        ("BEH_HEART_FAITH", "BEH_HEART_PIETY", True),  # Both salih
        # Opposite category (should be different)
        ("BEH_HEART_ARROGANCE", "BEH_CHAR_HUMILITY", False),  # Opposites
        ("BEH_HEART_FAITH", "BEH_CHAR_DISBELIEF", False),  # Opposites
    ]
    
    correct = 0
    for beh1, beh2, should_similar in test_pairs:
        concept1 = BEHAVIOR_TAXONOMY.get(beh1)
        concept2 = BEHAVIOR_TAXONOMY.get(beh2)
        if concept1 and concept2:
            emb1 = model.encode(concept1.arabic)
            emb2 = model.encode(concept2.arabic)
            sim = float(np.dot(emb1, emb2) / (np.linalg.norm(emb1) * np.linalg.norm(emb2)))
            is_similar = sim > 0.6
            if is_similar == should_similar:
                correct += 1
            logger.info(f"  {concept1.arabic} <-> {concept2.arabic}: sim={sim:.3f}")
...
# =============================================================================
# LAYER 3: BEHAVIORAL CLASSIFIER (33 classes)
# =============================================================================

def train_layer_3():
    """Train 33-class behavioral classifier."""
    logger.info("=" * 60)
    logger.info("LAYER 3: Behavioral Classifier (33 TRUE behavior classes)")
    logger.info("=" * 60)
    ...
    # Get unique behavior IDs and create mapping
...
# =============================================================================
# LAYER 4: RELATION EXTRACTOR (Usul-aligned)
# =============================================================================

def train_layer_4():
    """Train relation extractor with Quranic causal relations."""
    logger.info("=" * 60)
    logger.info("LAYER 4: Relation Extractor (Usul-aligned)")
    logger.info("=" * 60)
    ...
    # Relation types based on Quranic discourse
    RELATIONS = [
        "CAUSES",      # يسبب - A leads to B
        "PREVENTS",    # يمنع - A prevents B
        "OPPOSITE",    # ضد - A is opposite of B
        "INTENSIFIES", # يزيد - A intensifies B
        "LEADS_TO",    # يؤدي إلى - A results in B
        "NONE",        # No relation
    ]
    rel_to_id = {r: i for i, r in enumerate(RELATIONS)}
    
    # Known Quranic behavioral relations (expanded)
    KNOWN_RELATIONS = [
        # CAUSES - سبب
        ("BEH_HEART_HEEDLESSNESS", "BEH_HEART_ARROGANCE", "CAUSES"),
        ("BEH_HEART_ARROGANCE", "BEH_CHAR_OPPRESSION", "CAUSES"),
        ("BEH_HEART_ARROGANCE", "BEH_CHAR_DISBELIEF", "CAUSES"),
        ("BEH_CHAR_DISBELIEF", "BEH_CHAR_OPPRESSION", "CAUSES"),
        ("BEH_CHAR_POLYTHEISM", "BEH_CHAR_OPPRESSION", "CAUSES"),
        ("BEH_SPEECH_LYING", "BEH_HEART_HYPOCRISY", "CAUSES"),
        ("BEH_HEART_HEEDLESSNESS", "BEH_HEART_ARROGANCE", "CAUSES"),
        ("BEH_HEART_ENVY", "BEH_CHAR_OPPRESSION", "CAUSES"),
        
        # PREVENTS - يمنع
        ("BEH_HEART_PIETY", "BEH_CHAR_TRANSGRESSION", "PREVENTS"),
        ("BEH_WORSHIP_REMEMBRANCE", "BEH_HEART_HEEDLESSNESS", "PREVENTS"),
        ("BEH_HEART_FAITH", "BEH_CHAR_DISBELIEF", "PREVENTS"),
        ("BEH_CHAR_PATIENCE", "BEH_HEART_ANGER", "PREVENTS"),
        ("BEH_SPEECH_TRUTHFULNESS", "BEH_SPEECH_LYING", "PREVENTS"),
        ("BEH_CHAR_HUMILITY", "BEH_HEART_ARROGANCE", "PREVENTS"),
        ("BEH_CHAR_MERCY", "BEH_CHAR_OPPRESSION", "PREVENTS"),
        ("BEH_CHAR_JUSTICE", "BEH_CHAR_OPPRESSION", "PREVENTS"),
        ("BEH_WORSHIP_GRATITUDE", "BEH_CHAR_DISBELIEF", "PREVENTS"),
        
        # OPPOSITE - ضد
        ("BEH_SPEECH_TRUTHFULNESS", "BEH_SPEECH_LYING", "OPPOSITE"),
        ("BEH_HEART_FAITH", "BEH_CHAR_DISBELIEF", "OPPOSITE"),
        ("BEH_CHAR_HUMILITY", "BEH_HEART_ARROGANCE", "OPPOSITE"),
        ("BEH_CHAR_JUSTICE", "BEH_CHAR_OPPRESSION", "OPPOSITE"),
        ("BEH_WORSHIP_REMEMBRANCE", "BEH_HEART_HEEDLESSNESS", "OPPOSITE"),
        ("BEH_CHAR_TRUSTWORTHINESS", "BEH_CHAR_BETRAYAL", "OPPOSITE"),
        ("BEH_HEART_SINCERITY", "BEH_HEART_SHOWING_OFF", "OPPOSITE"),
        ("BEH_HEART_PIETY", "BEH_CHAR_TRANSGRESSION", "OPPOSITE"),
        
        # INTENSIFIES - يزيد
        ("BEH_HEART_ARROGANCE", "BEH_CHAR_DISBELIEF", "INTENSIFIES"),
        ("BEH_HEART_HEEDLESSNESS", "BEH_HEART_ARROGANCE", "INTENSIFIES"),
        ("BEH_CHAR_TRANSGRESSION", "BEH_CHAR_DISBELIEF", "INTENSIFIES"),
        
        # LEADS_TO - يؤدي إلى
        ("BEH_CHAR_DISBELIEF", "BEH_CHAR_OPPRESSION", "LEADS_TO"),
        ("BEH_HEART_FAITH", "BEH_HEART_PIETY", "LEADS_TO"),
        ("BEH_CHAR_PATIENCE", "BEH_HEART_CONTENTMENT", "LEADS_TO"),
        ("BEH_HEART_PIETY", "BEH_CHAR_EXCELLENCE", "LEADS_TO"),
        ("BEH_WORSHIP_REPENTANCE", "BEH_HEART_FAITH", "LEADS_TO"),
    ]
    ...
    # Positive examples with context augmentation
    for beh1, beh2, rel in KNOWN_RELATIONS:
        ...
        for ctx1 in ctx1_list if ctx1_list else [concept1.arabic]:
            for ctx2 in ctx2_list if ctx2_list else [concept2.arabic]:
                text = f"{concept1.arabic} [SEP] {concept2.arabic} [SEP] {ctx1} {ctx2}"
    ...
    for _ in range(len(examples)):
        ...
        if (b1, b2) not in known_pairs:
            concept1 = BEHAVIOR_TAXONOMY.get(b1)
            concept2 = BEHAVIOR_TAXONOMY.get(b2)
            if concept1 and concept2:
                ctx1 = random.choice(behavior_context.get(b1, [concept1.arabic]))
                ctx2 = random.choice(behavior_context.get(b2, [concept2.arabic]))
                text = f"{concept1.arabic} [SEP] {concept2.arabic} [SEP] {ctx1} {ctx2}"
                examples.append({"text": text, "label": rel_to_id["NONE"]})
                none_count += 1
    
    logger.info(f"Added {none_count} NONE relation examples")
...
# =============================================================================
# LAYER 5: GNN GRAPH REASONER (TRUE behaviors only)
# =============================================================================

def train_layer_5():
    """Train GNN on TRUE behavior co-occurrence graph."""
    logger.info("=" * 60)
    logger.info("LAYER 5: GNN Graph Reasoner (TRUE behaviors only)")
    logger.info("=" * 60)
    ...
    # Build co-occurrence graph from verses
    ...
    logger.info(f"Graph nodes: {len(behavior_ids)} TRUE behaviors")
    ...
    for beh_id in behavior_ids:
        concept = BEHAVIOR_TAXONOMY.get(beh_id)
        if concept:
            emb = embedder.encode(concept.arabic)
        else:
            emb = embedder.encode(beh_id)
    ...
    logger.info(f"Final AUC: {final_auc:.4f}, Best AUC: {best_auc:.4f}")
...
def train_layer_6():
    """Train domain reranker on TRUE behaviors."""
    logger.info("=" * 60)
    logger.info("LAYER 6: Domain Reranker (TRUE behaviors)")
    logger.info("=" * 60)
    ...
    for beh_id, docs in behavior_docs.items():
        ...
        for template in QUERY_TEMPLATES:
            query = template.format(behavior=concept.arabic)
...
def main():
    logger.info("=" * 70)
    logger.info("QBM USUL AL-FIQH ALIGNED TRAINING")
    logger.info("=" * 70)
    ...
    logger.info("")
    logger.info("=" * 70)
    logger.info("USUL-ALIGNED TRAINING SUMMARY")
    logger.info("=" * 70)
    ...
    # Save results
    with open(MODELS_DIR / "usul_training_results.json", 'w') as f:
